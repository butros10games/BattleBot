...
class AimAssist:
    ...
    def initialize_tracker(self):
        ...
        params.backbone = os.path.join(self.current_dir, 'docs', 'nanotrack_backbone.onnx')
    ...
    async def receive_frames(self):
        while True:
            ...
            while len(frame_data) < frame_length:
                packet = await self.loop.sock_recv(self.client_socket, frame_length - len(frame_data))
                if not packet:
                    break
                frame_data += packet
    ...
    async def process_loop(self):
        ...
        while True:
            ...
            if frame is not None:
                ...
                detection = await getattr(self, f"{self.aim_config['detection']}_detection")()
                x, y, w, h = detection
    ...
    def update_position_ratio(self, x, w):
        if self.tracking_started or self.object_detected:
            ...
            side = slice(None, roi_width) if self.position_ratio < 0.5 else slice(-roi_width, None)
            red_ratio = abs(self.position_ratio - 0.5)
            self.main_video[:, side, 2] = np.clip(self.main_video[:, side, 2] + int(255 * red_ratio), 0, 255)
    ...
    def get_aim_assist(self, x_angle):
        ...
        if self.steering_activated and (x_angle - self.aim_assist_range) <= x_camera <= (x_angle + self.aim_assist_range):
            x_angle = x_camera
        return x_angle
